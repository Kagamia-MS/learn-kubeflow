...
def convert_examples_to_features(examples, max_seq_length, tokenizer, label_list):
    ...
    df_feature = pd.DataFrame({'raw_text': raw_text_list, 'input_ids': input_ids_list, 'input_mask': input_mask_list,
                               'segment_ids': segment_ids_list, 'label_ids': label_ids_list})

    return df_feature


def convert_examples_to_features_nolabel(examples, max_seq_length, tokenizer):
    """Loads a data file into a list of `InputBatch`s."""
...
def main():
    ...
    df_features = convert_examples_to_features_nolabel(
